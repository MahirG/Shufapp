import {
  ArrowRight, Check, CheckCheck, Crown, Flame, Globe2, Heart, Languages, MessageSquare, Monitor, MoonStar, Phone,
  Search, ShieldCheck, Sparkles, Sun, Swords, Trophy, UserPlus, Users, Video, Volume2,
  VolumeX, WifiOff, X,
} from "lucide-react";
import { AnimatePresence, motion } from "motion/react";
import { ReactNode, useEffect, useMemo, useRef, useState } from "react";
import { FeedLanguage, Game, NotificationItem, ShufSettings, Suitor, View, modeCards, navItems, suitors } from "./data";
...
const languageOptions: { id: FeedLanguage; label: string }[] = [
...
export function SettingsModal({ settings, onChange, onClose }: { settings: ShufSettings; onChange: (next: ShufSettings) => void; onClose: () => void }) {
  return <Overlay title="Settings" eyebrow="Your Shuf experience" onClose={onClose}>
    <div className="settings-group"><span>APPEARANCE</span><div className="segmented-control">{([{ id: "system", icon: Monitor, label: "System" }, { id: "dark", icon: MoonStar, label: "Dark" }, { id: "light", icon: Sun, label: "Light" }] as const).map(({ id, icon: Icon, label }) => <button className={cn(settings.theme === id && "selected")} key={id} onClick={() => onChange({ ...settings, theme: id })}><Icon size={17} />{label}</button>)}</div></div>
    <div className="settings-group"><span>LANGUAGE & DATA</span><div className="settings-list"><label className="settings-select-row"><span><Languages size={18} /></span><div className="setting-copy"><strong>Feed language</strong><small>Choose what appears first in your home feed</small></div><select value={settings.language} onChange={(event) => onChange({ ...settings, language: event.target.value as FeedLanguage })}>{languageOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select></label><button onClick={() => onChange({ ...settings, lowData: !settings.lowData })}><span><WifiOff size={18} /></span><div className="setting-copy"><strong>Low Data Mode</strong><small>Pause large media and reduce visual effects</small></div><i className={cn("switch", settings.lowData && "on")}><b /></i></button></div></div>
    <div className="settings-group"><span>BEHAVIOR</span><div className="settings-list"><button onClick={() => onChange({ ...settings, sound: !settings.sound })}><span>{settings.sound ? <Volume2 size={18} /> : <VolumeX size={18} />}</span><div className="setting-copy"><strong>Sound effects</strong><small>Message and game feedback sounds</small></div><i className={cn("switch", settings.sound && "on")}><b /></i></button><button onClick={() => onChange({ ...settings, motion: !settings.motion })}><span><Sparkles size={18} /></span><div className="setting-copy"><strong>Motion and animation</strong><small>Use transitions and playful effects</small></div><i className={cn("switch", settings.motion && "on")}><b /></i></button><button onClick={() => onChange({ ...settings, messagePreview: !settings.messagePreview })}><span><MessageSquare size={18} /></span><div className="setting-copy"><strong>Message previews</strong><small>Show latest message in conversation list</small></div><i className={cn("switch", settings.messagePreview && "on")}><b /></i></button></div></div>
  </Overlay>;
}

export function AboutModal({ onClose }: { onClose: () => void }) {
  return <Overlay title="About Shuf" eyebrow="Ethiopia's social home" onClose={onClose}><div className="about-modal"><div className="about-logo"><Logo /><div><strong>SHUF</strong><span>Talk. Connect. Belong.</span></div></div><p>Shuf is a multilingual Ethiopian social network for community conversations, Circles, creators, events, messaging and interactive entertainment.</p><div className="about-grid"><button onClick={() => window.open("https://github.com/MahirG/Shufapp", "_blank", "noopener,noreferrer")}><ShieldCheck size={19} /><strong>Privacy and trust</strong><span>Phone numbers are private and every post includes local safety controls.</span></button><button onClick={() => navigator.clipboard?.writeText("https://github.com/MahirG/Shufapp")}><Globe2 size={19} /><strong>Built for Ethiopia</strong><span>Multilingual feeds, low-data support and community-led discovery.</span></button></div><small>Dating and matchmaking experiences remain opt-in entertainment features and do not control the public social feed.</small></div></Overlay>;
}

export function AchievementsModal({ onClose, onToast }: { onClose: () => void; onToast: (message: string) => void }) {
  const items = [{ icon: Flame, title: "On Fire", copy: "Contribute three days in a row", progress: "Unlocked", earned: true }, { icon: Swords, title: "No Mercy", copy: "Cast ten harsh verdicts", progress: "Unlocked", earned: true }, { icon: Heart, title: "Community Builder", copy: "Receive ten Respect reactions", progress: "Unlocked", earned: true }, { icon: Crown, title: "Circle Leader", copy: "Create 50 thoughtful community interactions", progress: "27 / 50", earned: false }];
  return <Overlay title="Achievements" eyebrow="Your reputation" onClose={onClose} wide><div className="achievement-modal-grid">{items.map(({ icon: Icon, title, copy, progress, earned }) => <button className={cn(earned && "earned")} key={title} onClick={() => onToast(earned ? `${title} selected` : `${23} more interactions to unlock Circle Leader`)}><span><Icon size={25} /></span><div><strong>{title}</strong><p>{copy}</p><small>{progress}</small></div>{earned ? <Check size={17} /> : <span className="mini-progress"><i style={{ width: "54%" }} /></span>}</button>)}</div></Overlay>;
}
...
export function SearchModal({ onClose, onNavigate, onOpenGame, onStartChat }: { onClose: () => void; onNavigate: (view: View) => void; onOpenGame: (game: Exclude<Game, null>) => void; onStartChat: (person: Suitor) => void }) {
  ...
  return <Overlay title="Search Shuf" eyebrow="People, Circles and experiences" onClose={onClose} wide><label className="command-search"><Search size={20} /><input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search people, Circles, games and sections" /><kbd>ESC</kbd></label><div className="command-results">{results.map(({ id, label, copy, icon: Icon, action }) => <button key={id} onClick={() => { action(); onClose(); }}><span><Icon size={18} /></span><div><strong>{label}</strong><small>{copy}</small></div><ArrowRight size={16} /></button>)}{!results.length ? <div className="command-empty"><Search size={24} /><strong>No result</strong><span>Try another person, Circle or topic.</span></div> : null}</div></Overlay>;
}
